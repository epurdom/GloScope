...
This vignette will review the steps needed to implement the `GloScope` methodology. `GloScope` is a framework for creating global profiles of scRNA-Seq samples in order to globally compare and analyze them across patients or tissue sites. First this methodology estimates a global gene expression distribution for each sample and then calculates how divergent pairs of samples are from each other. The output from the package's main function, `gloscope()`, is a $n\times n$ divergence matrix containing the pairwise statistical divergences between all samples. This divergence matrix can be the input to other downstream statistical and machine learning tools. 

`GloScope` estimates the gene expression density from a low dimensional embedding of the UMI count measurements, such as PCA or scVI. Users must provide `GloScope` with a `data.frame` containing each cell's embedding coordinates, along with the metadata which identifies to which sample each cell belongs to.
...
3. Calculate the symmetric KL divergence between all pairs of samples.
...
The data was a subset from the COVID PBMC data set, retrieved from @stephenson2021single, using [this hyperlinked URL](https://www.ebi.ac.uk/biostudies/arrayexpress/studies/E-MTAB-10026). The dataset has a total of 647,366 peripheral blood mononuclear cells (PBMCs) from 130 patients, whose phenotypes are COVID-infected, healthy control donor, patients with other non-COVID respiratory disease, and volunteers administered with intravenous lipopolysaccharide (IV-LPS). To enable faster computation, we subset this dataset to 20 random COVID and healthy control donors and further subsample each patient's count matrix to  500 random cells. This subsampled data is included as example data in the package. We emphasize that this subsampling procedure is only for demonstration purposes and is not a recommended step in normal analyses.

## Data Input

We first load the `GloScope` package and load the aforementioned example dataset.

```{r load-libs, message = FALSE,  warning = FALSE}
#library(GloScope) # TODO: Change me
devtools::load_all()
data("example_data")
```

The example data contains the first $50$ principal components of the subsampled cells, as well as the cell sample and phenotype labels. Note that each row of both `data.frames` corresponds to a single cell.
...
The default divergence for `GloScope` is the symmetric KL divergence, and the Jensen-Shannon divergence is also implemented. This can be controlled by setting `dist_mat="KL"` or `dist_mat="JS"`. One beneficial property of the Jensen-Shannon divergence is that its square root is a proper distance metric.

The following code demonstrates how the `gloscope` function can be called from the example data using both GMM and kNN approaches. Although the example data contains the first 50 principal components, we choose to use only the first 10 for calculations.
...
The `plotMDS` function provided by this package visualizes the output divergence matrix with multidimensional scaling. The `plotMDS` function utilizes the `isoMDS` function from the package `MASS`, and then `plotMDS` creates a scatter plot with samples color-coded by a user-specified covariate such as phenotype 

This function requires a `data.frame` with the relevant metadata at the sample level, rather than at the cell level. This can easily be obtained by appling the `unique` function from base R to the cell-level metadata `data.frame`. 
...
Here we plot the MDS representation with each sample color-coded by the `phenotype` variable. Note the function call returns both a matrix of MDS embeddings and a `ggplot` visualization. 
