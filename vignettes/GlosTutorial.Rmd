...
This package will go through each steps introduced in the paper (...) for the ``Glos`` framework. The output from the package would be a distance matrix summarizing the differences among different samples. Also, the function `plotMDS` in the package would allow the user to visualize the distance matrix.  
...
3. Approximate the KL divergence using Monte-Carlo estimation and obtain the divergence matrix of each sample pair.
...
The data was a subset from the Allenmouse data set, retrieved from [Yao et al (2021)](https://www.sciencedirect.com/science/article/pii/S0092867421005018?dgcid=rss_sd_all), using package https://github.com/drighelli/AllenInstituteBrainData. There was a total of 1,169,213 cells sequenced from 54 mice's different brain regions such as primary motor (Mop) region. For computational timing consideration, we subset the data to contains the samples from three regions: Anterior cingulate (ACA), Auditory (AUD), and primary motor (Mop). We further down-sampled the data so each sample has 500 cells.
