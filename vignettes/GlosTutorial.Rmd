...
The following is an example of how to set the random seed when running the `GMM` option, using the default of no parallelization (this code is not evaluated, so `eval=FALSE`):

```{r eval = F}
dist_test <- gloscope(example_data, sample_id = "patient_id", dim_redu = "PC",
                     ndim = 10, dens = "GMM", dist_mat = "KL",
